...
const SkillRadarChart = createWithRemoteLoader({
  ...
  withLocale(({ remoteModules, data }) => {
    ...
    const option = {
      ...
      series: [
        {
          name: formatMessage({ id: 'talentProfile.SkillComparison' }),
          type: 'radar',
          data: [
            {
              value: data.employee.map(item => parseInt(item.score)),
              name: formatMessage({ id: 'talentProfile.EmployeeStatus' }),
              itemStyle: {
                color: '#6366f1'
              },
              areaStyle: {
            ...
            {
              value: data.industry,
              name: formatMessage({ id: 'talentProfile.IndustryAverage' }),
              itemStyle: {
                color: '#f59e0b'
              },
              areaStyle: {
